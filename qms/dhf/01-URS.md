...
| 2 | Whisper deployment: on-prem or cloud? What is the PHI/de-identification posture? | **Resolved (2026-04-19)** | Dev: developer's choice (own API keys / self-hosted Whisper / any provider). Production: HIPAA-compliant via institutional BAA with **Azure AI Studio** (Whisper-compatible) or **AWS Bedrock**, whichever the deploying institution has an established agreement with. Both satisfy the SDS 04-03 §17 vendor boundary posture (minimum-necessary, zero-retention, region-pinned). See Cybersecurity §8 for the formalized vendor requirements. |
| 3 | Nomenclature arbitration owner: service director role, or separate workflow? | **Open** | Not a blocker for first-working-version. The override-quarantine queue UI is Stage-5 scope per Trace-Matrix §8.1; the arbitration-owner role will be surveyed from participating institutions when that UI is built. |
| 4 | LIS sign-out notification: push (HL7 event via interface) or module polls? | **Resolved (2026-04-19)** | Push, **orchestrator-mediated** — WILLET does not communicate with the LIS directly. Flow: WILLET saves finalized report to the **LORIS API** (orchestrator); the **Hermes interface engine** forwards via HL7 to the LIS; when the LIS signs out, the orchestrator receives the event and WILLET observes the transition during case synchronization. Separation of concerns preserved: WILLET does not subscribe to LIS events. |
| 5 | RTF document header: does the module emit accession number/patient header above parts, or does the LIS inject that from its own case record? | **Resolved (2026-04-19)** | WILLET emits **only the formatted-diagnosis component** as a self-contained RTF. The LIS assembles the ultimate record by combining this component with others it owns (gross description, accession header, patient demographics). The RTF serializer therefore does not render accession headers or patient identifiers; it produces the diagnosis payload only. |
| 6 | Autosave conflict resolution: what happens on network drop mid-save? Optimistic or pessimistic UI? What is the recovery model? | **Resolved (2026-04-19)** | **Autosave is default-on** (IntelliJ-style continuous save). A manual **Save button** is additionally available. A **user preference** toggles autosave on/off — default on. Network-drop recovery: edits continue to accumulate locally during DEGRADED state (SDS 04-00 §4.3 save state machine); on reconnect, edits replay via optimistic locking; version-mismatch conflicts trigger the explicit conflict UI (SDS 04-02 §6). |
...
| 8 | Maximum voice command length / recording timeout. | **Resolved (2026-04-19)** | **5 minutes** maximum per single dictation session. Visual warning at 30 seconds remaining. At cutoff, recording auto-stops and the current transcript is submitted. The user can immediately re-record to continue. The bound prevents runaway sessions from exhausting STT quotas and memory. |
| 9 | Synoptic protocol source: does WILLET maintain CAP protocol definitions internally, or fetch them from an external registry service? | **Open** | Not a blocker for first-working-version. Current internal JSON protocol fixtures are sufficient. External-registry integration is a Phase 2+ enhancement. |
| 10 | User preferences storage: new API endpoint on auth-system, or extension to existing user profile endpoint? | **Resolved (2026-04-19)** | **Hybrid per-module model.** Dev/standalone: local browser storage (or MSW-backed `/api/user/preferences`) — contributors iterate without an orchestrator. Production: the Starling orchestrator is authoritative; each module owns its own preference namespace (e.g., `willet.*`). The orchestrator fetches per-module preferences at module load and surfaces them through a unified Starling preferences UI. API contract: per-module-prefixed endpoints (details in `STARLING-MIS-001`). |
| 11 | Template source data: how are institutional templates authored and maintained? Admin UI, configuration file, or database management? | **Resolved (deferred, 2026-04-19)** | Near-term: templates remain **JSON documents** in `src/lib/data/protocols/` — current state is the "admin interface" for now. Long-term: a **Clinical Administration module** (separate, future Starling module) will provide authoring UX and emit updated JSON or an equivalent schema-backed store. Not a near-term WILLET deliverable. |
| 12 | Prompt panel auto-scroll: should new instruction entries auto-scroll the history log to the bottom, or should the pathologist control scroll position? | **Open (low priority)** | Default recommendation in effect: auto-scroll on new entry, pause on manual scroll-up. Revisit post-release based on pathologist feedback. |
