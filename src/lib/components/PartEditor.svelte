<script lang="ts">
  ...
  // Guard: only re-sync if the external finalDiagnosis differs from our local state,
  // so our own triggerSave round-trips don't clobber in-progress edits (blink bug fix).
  $effect(() => {
    const parsed = parseClauses(part);
    const localDiag = untrack(() => serializeClauses(clauses).finalDiagnosis);
    if ((part.finalDiagnosis ?? '') !== localDiag) {
      clauses = parsed;
    }
  });
  ...
  function triggerSave() {
    ...
    // This prevents the $effect from re-parsing stale data on the next tick.
...
<div class="rounded-lg border border-clinical-border bg-clinical-surface" onkeydown={handlePartKeydown}>
